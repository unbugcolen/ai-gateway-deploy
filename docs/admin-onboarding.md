...
本项目推荐使用“管理员直接发放 token”的模式：

1. 管理员在 LiteLLM 或 New API 后台创建员工 token。
2. 管理员把网关地址、个人 token、可用模型和访问要求发给员工。
3. 员工按 [employee-onboarding.md](employee-onboarding.md) 配置本地环境变量或 CC Switch。

邮件邀请、开放注册、邮箱白名单都不是必须项。只有当团队希望员工自己注册账号、自己登录后台管理个人 token 时，才需要考虑邮箱相关配置。
...
- 关闭开放注册。
- 默认不使用邮件邀请员工，也不开放员工自助注册。
- 如公司确实要开放自助注册，再限制邮箱域名白名单。
- 开启验证码或 Turnstile。
