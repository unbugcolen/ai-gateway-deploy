...
- **New API**：快速搭建中转站推荐方案。
- **LiteLLM + New API 组合方案**：LiteLLM 作为员工统一入口，New API 作为下游模型渠道池。
- **LiteLLM 混合路由方案**：Claude/Gemini/GPT 直连官方 API，DeepSeek/Qwen/Kimi 走 New API 渠道池。
...
如果希望两者组合，选 **LiteLLM + New API 组合方案**：员工、Claude Code、Codex 只访问 LiteLLM；LiteLLM 使用 New API 发放的服务 token 调 New API 的 OpenAI-compatible 入口；New API 负责供应商渠道池和后台点击配置。

如果团队已经确定 Claude/Gemini/GPT 走官方 key，同时希望 DeepSeek/Qwen/Kimi 由 New API 管渠道池，优先看 **LiteLLM 混合路由方案**。这比所有模型都绕 New API 更稳，也比纯 LiteLLM 更方便管理国内供应商渠道。
...
| LiteLLM 混合路由 | Claude/Gemini/GPT 官方直连 + New API 渠道池 | 强 | 强，Claude 官方直连 | 强 | 很强 | 强 | 强 | 已提供 | 同时承担 LiteLLM 与 New API 许可证义务 | 组合首选 |
| LiteLLM + New API | LiteLLM 入口 + New API 渠道池 | 强 | 强，经 LiteLLM 适配 | 强 | 很强 | 强 | 强 | 已提供 | 同时承担 LiteLLM 与 New API 许可证义务 | 组合推荐 |
...
│   ├── docker-compose.yml
│   └── .env.example
├── litellm-newapi-chain/
│   ├── docker-compose.yml
│   ├── .env.example
│   ├── litellm-config.yaml
│   ├── smoke-test.sh
│   └── README.md
├── litellm-hybrid-router/
│   ├── docker-compose.yml
...
│   ├── .env.new-api.example
│   ├── litellm-config.yaml
│   ├── litellm-observability.langfuse.snippet.yaml
